            ...
            <div class="col-md-6 col-xl-4 ml-auto mr-0">
                <form class="" id="sort_by_rating" action="{{ route('reviews.index') }}" method="GET">
                    <div class="" style="min-width: 200px;">
                        <select class="form-control sismoo-selectpicker" name="rating" id="rating" onchange="filter_by_rating()">
                ...
                @foreach($reviews as $key => $review)
                    @if ($review->product != null && $review->user != null)
                        <tr>
                            ...
                            <td><label class="sismoo-switch sismoo-switch-success mb-0">
    ...
    <script type="text/javascript">
        function update_published(el){
            ...
            $.post('{{ route('reviews.published') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    SISMOO.plugins.notify('success', '{{ translate('Published reviews updated successfully') }}');
                }
                else{
