...
class SismooUploadController extends Controller
{
    ...
    public function show_uploader(Request $request){
        return view('uploader.sismoo-uploader');
    }
    public function upload(Request $request){
        ...
        if($request->hasFile('sismoo_file')){
            $upload = new Upload;
            $extension = strtolower($request->file('sismoo_file')->getClientOriginalExtension());

            if(isset($type[$extension])){
                $upload->file_original_name = null;
                $arr = explode('.', $request->file('sismoo_file')->getClientOriginalName());
                ...
                $path = $request->file('sismoo_file')->store('uploads/all', 'local');
                $size = $request->file('sismoo_file')->getSize();
                ...
                if($type[$extension] == 'image' && get_setting('disable_image_optimization') != 1){
                    try {
                        $img = Image::make($request->file('sismoo_file')->getRealPath())->encode();
                        $height = $img->height();
                        $width = $img->width();
